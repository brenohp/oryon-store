...
// TODO: [BACKEND] Substituir estado local por gerenciador global (Context API/Zustand) sincronizado com o banco de dados.
const initialCarrinho = [
  { id: "1", nome: "Camiseta Oversized Soft Cotton", cor: "Off White", tamanho: "M", precoStr: "R$ 149,90", precoNum: 149.90, quantidade: 1, img: "/blusa-offwhite.jpg" },
  { id: "2", nome: "Camiseta Oversized Soft Cotton", cor: "Marrom", tamanho: "G", precoStr: "R$ 149,90", precoNum: 149.90, quantidade: 1, img: "/blusa-marrom.jpg" },
  { id: "3", nome: "Camiseta Oversized Soft Cotton", cor: "Preta", tamanho: "GG", precoStr: "R$ 149,90", precoNum: 149.90, quantidade: 1, img: "/blusa-preta.jpg" },
];

export default function Carrinho() {
  const [itensCarrinho, setItensCarrinho] = useState(initialCarrinho);

  // TODO: [BACKEND] Estas funções devem disparar actions globais e requisições para atualizar o carrinho no servidor (ex: PUT /api/carrinho/:id).
  const aumentarQuantidade = (id: string) => {
    setItensCarrinho(itensCarrinho.map(item => item.id === id ? { ...item, quantidade: item.quantidade + 1 } : item));
  };

  const diminuirQuantidade = (id: string) => {
    setItensCarrinho(itensCarrinho.map(item => item.id === id && item.quantidade > 1 ? { ...item, quantidade: item.quantidade - 1 } : item));
  };
  ...
  // TODO: [BACKEND] Integrar envio do carrinho para a API de checkout/pagamento.
  const handleCheckout = () => {
    console.log("Iniciando checkout com os itens:", itensCarrinho);
  };
          ...
          <div className="flex flex-col lg:flex-row gap-16">
              ...
              {itensCarrinho.map((item) => (
                <div key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center py-6 border-b border-oryon-black/10 relative group">
                  ...
                  <div className="relative w-24 h-32 md:w-32 md:h-40 bg-white/50 border border-oryon-black/5 flex-shrink-0 flex items-center justify-center mr-6 overflow-hidden">
                    <img src={item.img} alt={item.nome} className="w-full h-full object-contain p-2" />
                  </div>
                  ...
                  <div className="flex flex-col flex-grow w-full mt-4 sm:mt-0">
                    ...
                    <div className="flex items-center space-x-6">
                      <div className="flex items-center border border-oryon-black/20">
                        <button onClick={() => diminuirQuantidade(item.id)} className="px-3 py-1 font-sans text-sm text-oryon-black/50 hover:text-oryon-black hover:bg-oryon-black/5 transition-colors cursor-pointer">-</button>
                        <span className="font-sans text-xs font-bold w-6 text-center">{item.quantidade}</span>
                        <button onClick={() => aumentarQuantidade(item.id)} className="px-3 py-1 font-sans text-sm text-oryon-black/50 hover:text-oryon-black hover:bg-oryon-black/5 transition-colors cursor-pointer">+</button>
                      </div>

                      <button onClick={() => removerItem(item.id)} className="flex items-center space-x-2 font-sans text-[10px] font-bold tracking-[0.2em] uppercase text-oryon-black/40 hover:text-oryon-red transition-colors group/trash cursor-pointer">
            ...
            <div className="w-full lg:w-1/3">
              <div className="bg-oryon-black text-oryon-offwhite p-8 md:p-10 flex flex-col sticky top-8">
                ...
                <button onClick={handleCheckout} className="w-full group flex items-center justify-center space-x-4 border border-oryon-offwhite py-4 hover:bg-oryon-offwhite hover:text-oryon-black transition-colors duration-300 cursor-pointer">
